...
def LoadDmv():
   ...
   # assert use_cols in ['simple', 'content', 'multi', 'full', None], use_cols
    ...
    csv_file = './datasets/{}'.format('Vehicle__Snowmobile__and_Boat_Registrations.csv')
    cols = [
        'Record Type', 'VIN', 'Registration Class', 'City', 'State',
        'Zip', 'County', 'Model Year', 'Make', 'Body Type',
    ...
    return CsvTable('dmv', csv_file, cols, type_casts)

def LoadTpc():
    csv_file = './datasets/{}'.format('tpc.csv')
    cols = [2, 3, 4, 5, 6, 7]
    type_casts = {}
    return CsvTable('Tpc', csv_file, cols, type_casts, header=None, sep='|')
