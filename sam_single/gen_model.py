...
def Main():
    ...
    table_res_file = open('./generated_tables/{}.csv'.format(args.dataset), 'w', encoding="utf8")
    if args.dataset == 'dmv':
    ...
    while sampled_num < table.cardinality:
        sampled = model.sample(num=args.batch_size, device=DEVICE)
        sampled_num += args.batch_size
        sampled = sampled.cpu().numpy()
        for tuple in sampled:
            value_str = []
            for i in range(len(table.columns)):
                sample_value = table.columns[i].all_distinct_values[int(float(tuple[i]))]
                #print(sample_value)
                if pd.isnull(sample_value):
                    value_str.append('')
                else:
                    value_str.append(str(sample_value))

            value_str = ','.join(value_str)

            table_res_file.write(value_str + '\n')
            generated_tuple_num += 1

            if generated_tuple_num >= table.cardinality:
                break
