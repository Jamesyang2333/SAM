        ...
        for i in range(ncols):
            ...
            if not self.shortcircuit:
                ...
                if i == 0:
                    ...
                    probs_i_summed = torch.where(indicator_i == 0, ones_probs_i_summed,
                                                 probs_i_summed)  # [bs, num_samples]
                masked_probs.append(probs_i_summed)
                ...
                if self.shortcircuit:
                    ...
                    if natural_idx == 0:
                        normal_inp_candidate = self.model.EncodeInput(
                            ...
                            natural_col=0,
                            is_onehot=self.is_training)  # [bs*num_sample, encode_size of i]
                        inp_list[natural_idx] = normal_inp_candidate
                    else:
                        normal_inp_candidate = self.model.EncodeInput(one_hot_samples_i,
                                                                      natural_col=natural_idx,
                                                                      is_onehot=self.is_training)
...
class Oracle(CardEst):
    ...
    def Insert(self, columns, operators, vals):
        return 0

    def Delete(self, columns, operators, vals):
        return 0

    def Update(self, columns, operators, vals, updated_col_id, new_val):
        return 0
